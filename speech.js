export class ShadowLoop{
  constructor({onEcho,onState}){this.onEcho=onEcho;this.onState=onState;this.running=false;this.paused=false;this.timer=null;this.rate=1}
  play(text,{voice,rate=1}){this.stop(false);this.running=true;this.rate=rate;const speak=()=>{if(!this.running)return;const utterance=new SpeechSynthesisUtterance(text),started=performance.now();utterance.lang="ja-JP";utterance.voice=voice||null;utterance.rate=this.rate;utterance.onstart=()=>this.onState("speaking");utterance.onerror=event=>{if(event.error!=="canceled")this.onState("error");this.running=false};utterance.onend=()=>{if(!this.running)return;this.onEcho();const duration=Math.max(900,performance.now()-started);this.onState("imitate");this.timer=setTimeout(speak,duration+500)};speechSynthesis.speak(utterance)};speak()}
  setRate(rate){this.rate=rate;if(this.paused){speechSynthesis.resume();this.paused=false;this.onState("speaking")}return this.rate}
