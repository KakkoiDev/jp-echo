...
export async function pull({token, out, fetch: doFetch = fetch, sleep = ms => new Promise(r => setTimeout(r, ms)), log = () => {}}) {
  ...
  const since = previous?.meta?.scrubVersion === SCRUB_VERSION ? previous.meta.updatedAfter : null;
  const kanji = new Map((since && previous?.kanji || []).map(k => [k.id, k]));
  const vocabulary = new Map((since && previous?.vocabulary || []).map(v => [v.id, v]));

  let url = API + "?types=kanji,vocabulary" + (since ? "&updated_after=" + encodeURIComponent(since) : "");
  ...
  const result = {
    meta: {pulledAt: startedAt, updatedAfter: startedAt, incremental: !!since, scrubVersion: SCRUB_VERSION,
      kanji: kanji.size, vocabulary: vocabulary.size, source: "WaniKani API v2, personal use only"},
